...
const LEVEL_HISTORY_KEY=window.Suji.config.storage.levelHistory;
const HIGHEST_LEVEL_KEY=window.Suji.config.storage.highestLevel;
const VISITED_LEVELS_KEY=window.Suji.config.storage.visitedLevels;
...
const TYPE_COLORS = window.Suji.config.ui.pieceTypeColors;


const TUTORIAL_LEVELS = Object.fromEntries(Object.entries(window.Suji.levels.tutorial).map(([k,v])=>[k,v.message]));
const RULE_COPY = {
...
const {clamp,padLevel,mulberry32,shuffle}=window.Suji.core.utils;

const makeSudoku=window.Suji.game.makeSudoku;
const makePieces=window.Suji.game.makePieces;
