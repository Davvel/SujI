...
## v1.27.0 — protected checkpoint

- New protected checkpoint/rollback baseline created directly from the accepted **v1.26.5** build.
- **No gameplay, visual, layout, Hint, Rack, Board, drag-and-drop, or responsive behavior changed** during this checkpoint save.
- Retains the accepted v1.26.5 mobile-landscape balance: Board and Rack each receive at most 50% of usable play width on short touch landscape; the height-limited Board is centered in its half with breathing room; rotation recalculation remains active.
- Visible build number updated to **Version 1.27.0** in portrait and landscape.
- PWA service-worker cache namespace updated to **suji-v1-27-0**.
- `.gitattributes` remains included to preserve LF line endings.



## v1.29.0 — architecture migration from protected v1.27.0

- Rebased directly on the accepted/protected SuJi 1.27.0 gameplay implementation.
- Converted runtime startup to native browser ES modules (`js/main.js`).
- Added the `arch_1.1.0` folder boundaries: config, core, game, levels, data, layout, features, UI, storage and PWA.
- Added a first-class Levels subsystem with Tutorial, Standard, Themed/Photo-Pack and Sponsored adapters using a normalized LevelDefinition contract.
- Moved the ten validated tessellations into `js/data/pattern-provider.js`; JSON source copies now live under `data/patterns/`.
- Isolated scoring, Sudoku generation, puzzle construction, placement rules, Rack packing, responsive calculations, Hint workflow, drag/drop, tutorial, level picker, storage and PWA responsibilities into named modules.
- Preserved the accepted 1.27.0 CSS cascade in `css/base.css` for behavioural safety while adding component-owned CSS extension files and `config/theme.css`. The normal-mode landing-preview colour is now a theme token.
- Updated the visible version and PWA cache namespace to 1.29.0.

The protected 1.27.0 package remains the rollback baseline. This release is a refactor: gameplay changes are not intentional.
